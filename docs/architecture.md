...
### GitHub Actions
- **refresh.yml** — cron is `0 * * * *` (hourly), but GitHub only actually fires a fraction of
  those slots. Measured 2026-09-16 over a 122-hour window: 29 runs, all `schedule`, all success —
  a **~24% fire rate, averaging one run every ~4.2 hours** (observed gaps 2.2h–7.8h). Nothing is
  failing; GitHub drops scheduled runs under load, and `:00` is the most contended minute on the
  platform. Treat the feed as refreshing **every few hours, not hourly**. Runs build-cache.js and
  commits cache.json.
...
No Cloudflare Worker or PDF parsing is involved. The old Browser Rendering Worker
(`helltrack-results`) was retired and deleted from Cloudflare; its source is kept for reference
only at `archive/helltrack-results/` and is not deployed. The only live Worker is `helltrack-rss`.
...
### public/cache.json
Generated by GitHub Actions on the refresh.yml schedule — nominally hourly, in practice every
~4 hours (see refresh.yml note above). Structure:
```json
{
