...
- Cache refresh running clean (twice hourly, off the hour — see PBI 51)
...
| ~~Email platform migration away from Kit.com~~ | M | — | **Done** (2026-09-22). Now Loops, via its custom form endpoint rather than its hosted embed, so the markup is ours and the ~20 `!important` rules fighting Kit's stylesheet are gone. MailerLite was ruled out: its free plan dropped to 250 subscribers on 2026-06-16. The GA4 `email_signup` listener now keys on `.signup-section`, not a provider class, so the next migration cannot silently zero signup measurement. |
| ~~Welcome sequence on the new platform~~ | S | — | **Done** (2026-09-22). Loops workflow on contact-added, sending from hello@trackwalk.racing. Subject "You're in". Deliberately promises no cadence. |
...
| ~~Re-upload docs/icons to the Claude project~~ | XS | — | **Dropped** (2026-09-22). The Claude project is retired; Claude Code is the only interface. Its instructions were merged into `CLAUDE.md` and `docs/claude-project-instructions.md` was deleted. |
...
| 52 | ~~Feed image weight~~ | — | Done | **Shipped 2026-09-22.** The feed asked YouTube for `maxresdefault` (225KB, 1280x720) for every card and drew it 88px wide; 61 of 66 cache.json items point at it. `lazyImg()` now picks the variant from the class — `mqdefault` for row thumbs, `hqdefault` for the shorts strip (a portrait crop of a landscape still, `mq` is too soft at 3x), `maxresdefault` only for the hero, which really is ~1000px wide. Full feed scroll 13.1 MB → 1.3 MB. Rewritten at render, not in the pipeline, so it applies to cache.json already in every installed service worker. |
...
### Part B — make the app refresh itself while a round is live — SHIPPED 2026-09-16

Smaller than this spec assumed: `revalidateSeason()` already did the cache-busted fetch, the
no-op-when-unchanged diff, and the rules about not yanking the round out from under someone. It
only needed a `force` option to bypass its once-per-session guard, plus a timer and the guards
below — about 45 lines, not a rewrite.

Verified in-browser across six lifecycle cases: starts on a live round in the Results tab; stops
immediately on leaving the tab; restarts on return; stops when the round stops being Live; stays
off when the tab is hidden; stays off for a Provisional (past-dated) round. One fetch per poll.
Original plan follows, for Part C context.
